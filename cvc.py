...
def processImage(filename):
	img = cv2.imread('Img/captchas/' + filename, 0)
	...
	cv2.imwrite('Img/processedCaptchas/' + filename, imgBorder)

def findInCaptcha(filename):
	processImage(filename)
	elementFile = 'Img/elementsCaptcha/' + filename
	captchaFile = 'Img/processedCaptchas/' + filename
	...
	if template is None:
		cv2.imwrite('Img/elementsCaptcha/' + filename, img)
		...
		cv2.imwrite('Img/matchCaptcha/' + filename, found)
